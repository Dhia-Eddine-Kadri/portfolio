-- Migration 007: Add optional document_id filter to match_chunks_hybrid
-- This allows the frontend to pin retrieval to the specific file the student
-- is looking at, so the AI never searches the wrong document.
-- Run once in the Supabase SQL editor.

-- Drop old version (signature change requires DROP + CREATE)
...
CREATE OR REPLACE FUNCTION match_chunks_hybrid(
  ...
  p_document_id    uuid             DEFAULT NULL  -- optional: restrict to one document
)
RETURNS TABLE (
  id            uuid,
  document_id   uuid,
...
BEGIN
  RETURN QUERY
  WITH
  -- Semantic leg: cosine similarity via HNSW
  semantic AS (
    SELECT
      dc.id,
      dc.document_id,
  ...
  -- Keyword leg: full-text search via GIN
  keyword AS (
    SELECT
      dc.id,
      ROW_NUMBER() OVER (
