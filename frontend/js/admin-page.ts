...
async function refreshToken(): Promise<string | null> {
  ...
  if (!refresh || !supabaseUrl || !anonKey) return null;
...
async function verifyAdmin(retried = false): Promise<boolean> {
  if (!token) return false;
  window._sbToken = token;
  const status = await checkAdminStatus() as { isAdmin?: boolean } | null;
  if (status?.isAdmin) return true;
  if (!retried) {
    token = await refreshToken();
    if (token) return verifyAdmin(true);
  }
  return false;
}
...
async function mountAdmin(): Promise<void> {
  ...
  if (!admin || !styles || !root) throw new Error('Admin dashboard markup is unavailable');
  document.head.appendChild(styles);
  admin.style.display = '';
  root.replaceChildren(admin);
  window.showToast = (title: string, subtitle?: string) => {
...
mountAdmin().catch(() => {
  if (root) root.innerHTML = '<div class="admin-state">We could not load the admin dashboard right now.</div>';
});
