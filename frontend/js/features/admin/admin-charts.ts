...
// Mirrors the standalone Business-Control-Center preview: labelled axes, grid
// lines, a floating hover tooltip, a profit area fill and a peak marker.
//
// Each renderer clears `host`, injects a responsive <svg viewBox> plus an
// absolutely-positioned tooltip, and wires hover handlers. Hosts must be
// position:relative (the renderer sets it defensively).
...
function _prep(host: HTMLElement): { svg: SVGSVGElement; tip: HTMLDivElement } {
  ...
  svg.style.cssText = 'width:100%;height:260px;display:block;overflow:visible';
...
export function renderBarChart(
  host: HTMLElement,
  points: BarPoint[],
  opts: { caption?: string; tooltipNoun?: string } = {}
): void {
  const { svg, tip } = _prep(host);
  const m = { top: 24, right: 30, bottom: 40, left: 46 };
  ...
  // Whole-number y ticks (cap at ~6 lines so it stays clean).
  ...
  for (let i = 0; i <= max; i += tickStep) {
    ...
    html += '<text class="adm-axis" x="' + (m.left - 10) + '" y="' + (yy + 4) + '" text-anchor="end">' + i + '</text>';
  }
  ...
  points.forEach((p, i) => {
    ...
    if (i % every === 0 || i === points.length - 1) {
      html += '<text class="adm-axis" x="' + (x + barW / 2) + '" y="' + (H - 14) + '" text-anchor="middle">' + _esc(p.label) + '</text>';
    }
    if (p.value === maxVal && maxVal > 0) {
      html += '<text class="adm-peak" x="' + (x + barW / 2) + '" y="' + (y(p.value) - 8) + '" text-anchor="middle">Peak</text>';
    }
...
export function renderLineChart(
  ...
  const { svg, tip } = _prep(host);
  const m = { top: 22, right: 30, bottom: 40, left: 58 };
  const iw = W - m.left - m.right;
  const ih = H - m.top - m.bottom;
  ...
  for (let i = 0; i <= ticks; i++) {
    ...
    const yy = y(v);
    html += '<line class="adm-gridline" x1="' + m.left + '" x2="' + (W - m.right) + '" y1="' + yy + '" y2="' + yy + '"/>';
    html += '<text class="adm-axis" x="' + (m.left - 10) + '" y="' + (yy + 4) + '" text-anchor="end">' + _esc(fmt(v)) + '</text>';
  }
  html += '<line class="adm-axis-line" x1="' + m.left + '" x2="' + (W - m.right) + '" y1="' + y(0) + '" y2="' + y(0) + '"/>';
  points.forEach((d, i) => {
    html += '<text class="adm-axis" x="' + x(i) + '" y="' + (H - 14) + '" text-anchor="middle">' + _esc(d.label) + '</text>';
  });

  html += '<path class="adm-area" d="' + area(profit, y(0)) + '"/>';
  ...
  svg.querySelectorAll<SVGRectElement>('rect[data-i]').forEach((r) => {
    r.addEventListener('mousemove', (e) => {
      const d = points[Number(r.dataset['i'])];
      if (!d) return;
      _moveTip(host, tip, e.clientX, e.clientY,
