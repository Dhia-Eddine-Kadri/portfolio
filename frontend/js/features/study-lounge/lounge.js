export function initStudyLounge() {
    ...
    function loungeRender() {
        ...
        const resetBtn = document.getElementById('loungeResetBtn');
    ...
    window._statsTrackFile = function (fileName, courseName) {
        const stats = getStats();
        touchStreak();
        if (fileName && stats.filesOpened.indexOf(fileName) < 0)
            stats.filesOpened.push(fileName);
