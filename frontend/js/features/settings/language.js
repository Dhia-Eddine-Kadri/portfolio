...
export function applyLanguage(lang) {
    ...
    window._lang = _lang;
    window._t = (key) => (translations[_lang] || translations.en)[key] || key;
    ...
    if (obSub) {
        ...
        if (key)
            obSub.textContent = tr[key] || obSub.textContent;
    }
    const ssb = document.getElementById('saveSettingsBtn');
    if (ssb)
        ssb.textContent = tr.settings_save_btn || ssb.textContent;
