...
async function _generateWithRag(tool, level, topic) {
    ...
    if (level && (tool === 'quiz' || tool === 'summary'))
        label += ' (' + level + ')';
    if (window.addUserMsg)
        window.addUserMsg(label);
    _ragSetGenerating(true);
    if (window.addTyping)
        window.addTyping();
    try {
        const countMap = { easy: 5, medium: 6, hard: 8 };
        const result = (await generateStudyTool(courseId, tool, {
        ...
        let md;
        if (tool === 'flashcards')
            md = _renderFlashcards(result.items, result.sources);
        else if (tool === 'quiz')
            md = _renderQuiz(result.items, result.sources);
        else
            md = _renderSummary(result.text, result.sources);
        if (result.error)
            md = '⚠️ ' + result.error;
        document.querySelectorAll('.typing-wrap').forEach((el) => el.remove());
        _ragSetGenerating(false);
        if (!_ragAborted && window.addBotMsg)
            window.addBotMsg(md);
        return !_ragAborted;
    }
    catch {
...
export function chipPrompt(type, level) {
    ...
    if (!hasDoc && window.addBotMsg && window._t)
        window.addBotMsg(window._t('ai_tip_no_pdf'));
    const promptMap = {
        summarise: {
            small: base +
