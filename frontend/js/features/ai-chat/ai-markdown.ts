...
export function renderMarkdown(text: string): string {
  ...
  function inline(s: string): string {
    s = s.replace(/\\\(([^]*?)\\\)/g, (_, m: string) => renderKatex(m, false));
    s = s.replace(/\$\$([^]*?)\$\$/g, (_, m: string) => renderKatex(m, true));
    s = s.replace(/\$([^$\n]+?)\$/g, (_, m: string) => renderKatex(m, false));
    s = s.replace(/`([^`]+)`/g, (_, c: string) => '<code>' + esc(c) + '</code>');
