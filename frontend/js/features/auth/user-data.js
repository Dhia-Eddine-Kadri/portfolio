    ...
    try {
        try {
            ...
            if (cached) {
                ...
                if (cp && cp.full_name && window.applyProfile)
                    window.applyProfile(cp);
                if (cp && cp.courses && window._loadUserCourses)
                    window._loadUserCourses(cp.courses);
            }
        }
        catch {
        ...
        if (!sb)
            return;
        const profile = (await sb.from('profiles').select('*').eq('id', uid).single());
        if (profile && profile.full_name) {
            ...
            if (window.applyProfile)
                window.applyProfile(profile);
        }
        if (profile && profile.courses) {
            if (window._loadUserCourses)
                window._loadUserCourses(profile.courses);
        }
        ...
        const settings = (await sb.from('settings').select('*').eq('id', uid).single());
        if (settings && window.applySettings)
            window.applySettings(settings);
        let sub = (await sb.from('subscriptions').select('*').eq('user_id', uid).single());
        if (sub && sub.expires_at && Date.parse(sub.expires_at) <= Date.now()) {
            sub = { ...sub, status: 'expired' };
        }
        if (window.applySubscription)
            window.applySubscription(sub || {});
        ...
        checkAdminStatus()
            ...
            .catch(() => {
            if (!window._userIsPro && window._showPaywall)
                setTimeout(window._showPaywall, 800);
        });
        ...
        if (typeof window._dwLoadAndRender === 'function')
...
export function applyProfile(p) {
    ...
    const u = document.getElementById('profileUniversity');
    const pr = document.getElementById('profileProgramme');
    const pv = document.getElementById('profileVertiefung');
    const m = document.getElementById('profileMatrikel');
    ...
    if (e && p.email)
        e.value = p.email;
    if (u && p.university)
        u.value = p.university;
    if (pr && p.programme)
        pr.value = p.programme;
    if (pv && p.vertiefung)
        pv.value = p.vertiefung;
    if (m && p.matrikel)
        m.value = p.matrikel;
    if (i && p.full_name)
        i.textContent = p.full_name.charAt(0).toUpperCase();
    if (p.vertiefung) {
    ...
    if (p.chat_username)
        window._chatUsername = p.chat_username;
...
export function applyUserTypeUI() {
    ...
    if (coursesNav)
        coursesNav.style.display = isLearner ? 'none' : '';
    if (germanNav)
        germanNav.style.display = isLearner ? '' : 'none';
    const glSub = document.getElementById('glTestBadge');
    const glChip = document.getElementById('glLevelChip');
    if (glSub)
        glSub.textContent = (germanTest || 'German Test') + ' preparation';
    if (glChip)
        glChip.textContent = germanLevel || '–';
    document.querySelectorAll('.pf-enrolled-field').forEach((el) => {
        el.style.display = isLearner ? 'none' : '';
    });
    document.querySelectorAll('.pf-learner-field').forEach((el) => {
        el.style.display = isLearner ? '' : 'none';
    });
    const gt = document.getElementById('profileGermanTest');
    const gl = document.getElementById('profileGermanLevel');
    if (gt && germanTest)
        gt.value = germanTest;
    if (gl && germanLevel)
        gl.value = germanLevel;
}
