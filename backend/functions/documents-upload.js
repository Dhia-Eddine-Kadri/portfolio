...
exports.handler = async function (event) {
  ...
  // Indexing runs in Python. If unreachable, the document stays in
  // `processing_status: 'uploaded'` and the next index-existing request
  // (or a retry from the UI) will pick it back up.
  if (pythonAiConfigured()) {
    ...
    if (!py.ok) {
      console.warn('[documents-upload] Python indexing failed:', py.status, py.body && py.body.error);
    }
  } else {
