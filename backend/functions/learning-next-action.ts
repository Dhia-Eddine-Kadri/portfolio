...
// Proactive "what should I study next" recommendation (Learning Agent Core,
// Phase 1) — combines weak-topic memory with the course Topic Map.
...
export const handler = async (event: NetlifyEvent): Promise<LambdaResponse> => {
  ...
  const r = await forwardToPython('learning/next-action', { userId: user.id, courseId });
  if (!r.ok) {
