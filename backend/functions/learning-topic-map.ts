// POST /api/learning/topic-map
// Returns the stored per-course Topic Map (Learning Agent Core, Phase 1).
// Auth here (Supabase JWT); the Python service cross-checks course ownership.
...
export const handler = async (event: NetlifyEvent): Promise<LambdaResponse> => {
  ...
  const r = await forwardToPython('course-topic-map', { userId: user.id, courseId });
  if (!r.ok) {
